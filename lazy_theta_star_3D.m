...
function [path,n_points]=lazy_theta_star_3D(K,E3d_safe,x0_old,y0_old,z0_old,xend_old,yend_old,zend_old,sizeE)
...
while isempty(open)==0 && exit_path==0
    ...
    %Line of sight check between coming point (parent) and current point
    sight=line_sight_partial_3D(E3d_safe,xb,yb,zb,sizeE); 
    
    
    %If there is no line of sight
    if sight==0 || sight==0.5
            ...
            for j=-1:1
                for k=-1:1                    
                    ...
                    if xcurrent+i>0 && ycurrent+j>0 && zcurrent+k>0 && xcurrent+i<=x_size && ycurrent+j<=y_size && zcurrent+k<=z_size
                        ...
                        %If the neighbour node is within the closed list
                        if max(sum([ycurrent+j==closed(:,1) xcurrent+i==closed(:,2) zcurrent+k==closed(:,3)],2))==3
                                                        
                            %Evaluate the distance from start to the neighbour node + from the neighbour node to the current node
                            g_test=G(ycurrent+j,xcurrent+i,zcurrent+k)+sqrt(i^2+j^2+k^2);                            
                                                        
                            %If this distance is smallest, save it and assign neighbour as parent node
                            if g_test<g_min
                                
                                g_min=g_test;  
                                G(ycurrent,xcurrent,zcurrent)=g_test;
                ...
                for k=-1:1                    
                    ...
                    if xcurrent+i>0 && ycurrent+j>0 && zcurrent+k>0 && xcurrent+i<=x_size && ycurrent+j<=y_size && zcurrent+k<=z_size
                        ...
                        if check_open<3 && check_closed<3                                                   
                            ...
                            g_try=G(came_fromy(ycurrent,xcurrent,zcurrent),came_fromx(ycurrent,xcurrent,zcurrent),came_fromz(ycurrent,xcurrent,zcurrent))+sqrt((came_fromy(ycurrent,xcurrent,zcurrent)-(ycurrent+j))^2+(came_fromx(ycurrent,xcurrent,zcurrent)-(xcurrent+i))^2+(came_fromz(ycurrent,xcurrent,zcurrent)-(zcurrent+k))^2);
